...
helm upgrade --install app-simulator $APP_SIMULATOR_CHART_DIR -f receiver.yaml -n $B_CLUSTER_NAMESPACE --set db.appSimulator.password=$POSTGRES_ADMIN_PASSWORD --set appSimulators.receiver.topicCreation.replicationFactor=$KAFKA_REPLICATION_FACTOR --set "imagePullSecrets={docker-registry-cred}" --set image.tag=$DOCKER_IMAGE_VERSION --set "db.appSimulator.namespace=$APP_SIMULATOR_DB_NAMESPACE"  --wait
